...
- 防洪排涝通道：来自 `防洪排涝通道.geojson`，1,208 段线、1,273 个点，默认打开。
- 巫山排水缺陷分布：来自 `巫山缺陷分布图.geojson`，107,607 段线、42,013 个点，默认打开。
...
- 当前供水、防洪排涝、排水缺陷均优先使用标准 GeoJSON 线几何。`.ovobj` 处理脚本保留在目录中，作为无法导出标准格式时的备用方案。
- 默认底图为 OpenStreetMap / Esri WGS84 底图，避免高德、百度等偏移底图造成错位。
